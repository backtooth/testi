...
function info(s,x,y){
 ...
 else if(side){const key=side+lane;kind=(depth===2&&s.moats.includes(key))||(depth===1&&s.widened.includes(key))?'water':'road';}
 else if((x===2||x===8)&&(y===2||y===8))kind='water';
 return {x,y,id,kind,side,lane,depth,troops:s.troops.filter(t=>{const p=unitPos(t);return p.x===x&&p.y===y;})};
}
...
function tile(c,s,x,y,size=32,units=true){const f=info(s,x,y);c.save();c.scale(size/32,size/32);rect(c,P.grass,0,0,32,32);for(let i=0;i<14;i++){const a=(x*7+y*13+i*11)%30,b=(x*19+y*3+i*7)%30;rect(c,i%2?P.grassLight:P.grassDark,a,b,2,1);}
 ...
 if(units&&f.troops.length){const shown=f.troops.slice(0,3);shown.forEach((t,i)=>soldier(c,t.type,3+i*6,8-i*3,.85));}
 c.restore();return f;}
...
const names={grass:'Nurmikenttä',road:'Hyökkäyslinja',water:'Vallihauta',court:'Linnan sisäpiha',wall:'Muuri',tower:'Kulmatorni',gate:'Portti',ruin:'Tornin rauniot'};
function inspect(s,active){const canvas=document.getElementById('tile-scene'),c=canvas.getContext('2d');c.imageSmoothingEnabled=false;c.clearRect(0,0,288,192);const f=info(s,active.x,active.y);
 ...
 const title=document.getElementById('inspect-title');title.textContent='X'+(active.x+1)+', Y'+(active.y+1)+' · '+names[f.kind];
 ...
 if(f.kind==='water'){stat('Ylitys','1 odotusvuoro / vesiruutu');if(f.side)stat('Haudan leveys',s.widened.includes(f.side+f.lane)?'2 ruutua':'1 ruutu');}
 ...
 f.troops.forEach(t=>{const row=document.createElement('div');row.className='inspect-unit';const icon=document.createElement('canvas');icon.width=24;icon.height=22;icon.setAttribute('aria-hidden','true');soldier(icon.getContext('2d'),t.type,2,2);const name=document.createElement('strong');name.textContent='#'+t.id+' '+E.cards[t.type].name;const values=document.createElement('span');values.textContent=t.hp+'/'+t.max+' HP · isku '+t.damage+(t.waiting?' · ODOTTAA VEDESSÄ':'');const bar=document.createElement('progress');bar.max=t.max;bar.value=t.hp;bar.setAttribute('aria-label','#'+t.id+' kestävyys');const text=document.createElement('div');text.append(name,values,bar);row.append(icon,text);list.appendChild(row);});
