...
function tile(c,s,x,y,size=32,units=true){const f=info(s,x,y);c.save();c.scale(size/32,size/32);rect(c,P.grass,0,0,32,32);for(let i=0;i<14;i++){const a=(x*7+y*13+i*11)%30,b=(x*19+y*3+i*7)%30;rect(c,i%2?P.grassLight:P.grassDark,a,b,2,1);}
 ...
 if(units&&f.troops.length){const shown=f.troops.slice(0,3);shown.forEach((t,i)=>{soldier(c,t.type,3+i*6,8-i*3,.85);if(t.oily){rect(c,'#171323',5+i*6,15-i*3,5,6);rect(c,'#171323',8+i*6,12-i*3,2,3);}if(t.burning)flame(c,4+i*7,6-i*2);if(t.skip){rect(c,'#ffe088',3+i*7,2,5,5);rect(c,'#443828',5+i*7,2,1,3);}});}
