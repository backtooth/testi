(() => {
 ...
 function use(fallback=false){if(!E.play(s,s.role,selected,target,fallback))return;selected=0;if(s.role==='attack'){const t=s.troops[s.troops.length-1];if(t)activeCell=PixelSiege.unitPos(t);}if(s.role==='defend'&&s.phase==='attack')aiHand('attack');render();}
 ...
 function position(t){const point=route(t.side,t.lane,t.pos);return {x:Math.floor(point[0]/100)+1,y:Math.floor(point[1]/100)+1};}
 function coordinate(t){const p=position(t);return 'X'+p.x+', Y'+p.y;}
 function axes(){for(const axis of ['x','y']){const host=$(axis+'-axis');host.replaceChildren();for(let n=1;n<=11;n++){const label=document.createElement('span');label.textContent=n;label.setAttribute('aria-label',axis.toUpperCase()+n);host.appendChild(label);}}}
 function defenseUI(){
  ...
  shown.forEach(o=>{const div=document.createElement('div');div.style.borderLeftColor=colors[o.corner];div.className='order-key';div.textContent=(orders.length?'':'Edellinen: ')+E.towerNames[o.corner]+' → #'+o.troopId+(s.troops.find(t=>t.id===o.troopId)?' ['+coordinate(s.troops.find(t=>t.id===o.troopId))+']':'')+' '+E.sideNames[o.side]+' '+o.lane;summary.appendChild(div);});
 ...
 function map(){
  ...
  for(let y=0;y<11;y++)for(let x=0;x<11;x++){
   ...
   const cell=document.createElement('button');cell.type='button';cell.className='cell';cell.dataset.x=x+1;cell.dataset.y=y+1;cell.title='X'+(x+1)+', Y'+(y+1);
   if(edge){const wall=s.walls[id],tower=Object.hasOwn(s.towers,id);cell.classList.add('wall');if(tower){cell.dataset.corner=id;if(viewTower===id)cell.classList.add('selected-tower');}if(id===22)cell.classList.add('gate');if(wall.hp<wall.max)cell.classList.add('damaged');const sym=document.createElement('span');sym.textContent=tower?'♜':Object.hasOwn(E.towerNames,id)?'×':id===22?'▥':'▤';const hp=document.createElement('small');hp.textContent=wall.hp+'/'+wall.max;cell.append(sym,hp);cell.setAttribute('aria-label',(tower?E.towerNames[id]+' kulmatorni, ':Object.hasOwn(E.towerNames,id)?'Tornin rauniot, ':id===22?'Portti, ':'Muuri, ')+E.sideNames[lane.side]+' '+lane.lane+', kestävyys '+wall.hp+'/'+wall.max);}
   else if(castle){cell.classList.add('court');if(x===5&&y===5)cell.textContent='⚑';}
   else if(lane){cell.classList.add('path');cell.dataset.side=lane.side;cell.dataset.lane=lane.lane;cell.dataset.depth=lane.depth;cell.textContent=lane.depth===0?({N:'↓',E:'←',S:'↑',W:'→'}[lane.side]+lane.lane):'·';if(lane.depth===0)cell.classList.add('arrow');if((lane.depth===2&&s.moats.includes(E.laneKey(lane.side,lane.lane)))||(lane.depth===1&&s.widened.includes(E.laneKey(lane.side,lane.lane)))){cell.classList.add('chasm');cell.textContent='';if(s.water.includes(E.cellKey(lane.side,lane.lane,lane.depth)))cell.classList.add('moat');}cell.setAttribute('aria-label',E.sideNames[lane.side]+' linja '+lane.lane+', '+(3-lane.depth)+' ruutua muurista');}
   else if((x===2||x===8)&&(y===2||y===8)){cell.classList.add('chasm');cell.textContent='';}else cell.classList.add('empty');
   if(lane){
    ...
    cell.onclick=()=>{activeCell={x,y};selectCell({...lane,depth:edge?3:lane.depth},id);};
   ...
   if(lane)cell.setAttribute('aria-label','X'+(x+1)+', Y'+(y+1)+' · '+cell.getAttribute('aria-label'));
   if(!lane){cell.setAttribute('aria-label','X'+(x+1)+', Y'+(y+1));cell.onclick=()=>{activeCell={x,y};target.depth=-1;render();};}
   const caption=document.createElement('div');caption.className='tile-caption';while(cell.firstChild)caption.appendChild(cell.firstChild);cell.appendChild(PixelSiege.boardTile(s,x,y));cell.appendChild(caption);if(activeCell.x===x&&activeCell.y===y)cell.classList.add('active-cell');
 ...
 function render(){
  if(viewTower!==null&&!Object.hasOwn(s.towers,viewTower))viewTower=null;
  if(!Object.hasOwn(s.towers,target.corner))target.corner=Number(Object.keys(s.towers)[0]??0);
  for(const option of $('corner').options)option.disabled=!Object.hasOwn(s.towers,option.value);
  ...
  $('target-info').textContent=(gate?'Portti':Object.hasOwn(E.towerNames,E.tile(target.side,target.lane))?(wall.hp?'Torni':'Tornin rauniot'):'Muuri')+' '+wall.hp+'/'+wall.max+' · '+(gate?'Portin kulkutie':s.water.includes(E.cellKey(target.side,target.lane,target.depth??2))?'Valittu ruutu: vesi':'Valittu ruutu: kuiva rotko / lähestyminen');
  ...
  const hand=$('hand');hand.replaceChildren();s.hands[s.role].forEach((key,i)=>{const b=document.createElement('button');b.className='card'+(i===selected?' selected':'');b.type='button';b.disabled=!active;b.setAttribute('aria-pressed',String(i===selected));const strong=document.createElement('strong');strong.textContent=E.cards[key].name;const small=document.createElement('small');small.textContent=E.cards[key].text;b.append(PixelSiege.icon(key),strong,small);b.onclick=()=>{selected=i;render();};hand.appendChild(b);});
  $('corner-wrap').hidden=!(active&&card==='upgrade');$('play').hidden=!active;$('play').disabled=!card||!E.valid(s,card,target);$('fallback').hidden=!(active&&s.role==='defend');$('fallback').disabled=!E.valid(s,'reinforce',target);
  $('help').textContent=!active?'':card==='moat'&&!E.valid(s,card,target)?'Rotkoa voi leventää kerran. Ei portille.':card==='upgrade'?'Valitse parannettava kulmatorni kentältä tai valikosta.':'Pelaa kaikki kolme korttia yksi kerrallaan. Voit valita jokaiselle oman kohteen.';
  if(active&&['oil','fire','water'].includes(card))$('help').textContent=E.cards[card].text+(!E.valid(s,card,target)?' Valittu ruutu ei kelpaa kohteeksi.':' Kohde: '+E.sideNames[target.side]+' '+target.lane+', etäisyys '+(3-(target.depth??2))+' ruutu(a).');
